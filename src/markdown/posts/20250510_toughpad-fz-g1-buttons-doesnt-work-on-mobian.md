...
お久しぶりです。  
ブログの更新をずっとサボっていました。ネタがないわけではないんですが遅筆ゆえに書くのが億劫になってしまっていて、お蔵入りしている記事がいくつか・・・。書き途中のものを一ヶ月後とかに書き加えていこうとしてもできないらしいということに最近気づきました。一ヶ月も前の自分なんてほぼ他人ですからね。今月から一ヶ月に1回くらい投稿したいなという、そういう気持ちです。
...
ここまでで`TBTN`の`_HID`の返り値は32ビット圧縮EISAタイプIDと分かったので先ほどの`TBTN`の`_HID`の2つの返り値をデコードするスクリプトを書きます。
